module "level0" {
  ...
  fw_rg_enabled = var.level0_fw_rg_enabled
  la_rg_enabled = var.level0_la_rg_enabled
...
resource "azurerm_virtual_network" "firewall" {
  ...
  location            = module.level0.level0_firewall_location.*.location[count.index]
  resource_group_name = module.level0.level0_firewall_rg_name.*.name[count.index]
